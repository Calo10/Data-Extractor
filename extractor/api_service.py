...
from epitomax_client import EpitomaxSession
from models import EpitomaxAuthRequest, EpitomaxDownloadRequest
import base64
from epitomax_selenium import download_epitomax_pdf_base64
from models import EpitomaxSeleniumDownloadRequest, FTPConfig, EpitomaxJobStatus
...
epitomax_job_tracker = {}

@app.post("/DownloadEpitomaxFormsSelenium")
def download_epitomax_forms_selenium(request: EpitomaxSeleniumDownloadRequest):
    job_id = str(uuid4())
    status = EpitomaxJobStatus(job_id=job_id, status="pending", progress=0.0, message="Job created", start_time=datetime.now())
    epitomax_job_tracker[job_id] = status

    def run_job():
        try:
            epitomax_job_tracker[job_id].status = "running"
            epitomax_job_tracker[job_id].progress = 10.0
            result = download_epitomax_pdf_base64(
                request.username,
                request.password,
                request.url,
                output_destination=request.output_destination,
            ...
            epitomax_job_tracker[job_id].status = "completed"
            epitomax_job_tracker[job_id].progress = 100.0
            epitomax_job_tracker[job_id].end_time = datetime.now()
            if 'local_path' in result:
                epitomax_job_tracker[job_id].local_path = result['local_path']
            if 'ftp_host' in result:
                epitomax_job_tracker[job_id].ftp_host = result['ftp_host']
                epitomax_job_tracker[job_id].ftp_directory = result['ftp_directory']
            epitomax_job_tracker[job_id].message = result.get('message', '')
        except Exception as e:
            epitomax_job_tracker[job_id].status = "failed"
            epitomax_job_tracker[job_id].progress = 100.0
            epitomax_job_tracker[job_id].end_time = datetime.now()
            epitomax_job_tracker[job_id].errors.append(str(e))
            epitomax_job_tracker[job_id].message = "Job failed"

    Thread(target=run_job).start()
    return {"job_id": job_id, "status": "started"}

@app.get("/epitomax_status/{job_id}")
def get_epitomax_job_status(job_id: str):
    status = epitomax_job_tracker.get(job_id)
    if not status:
        raise HTTPException(status_code=404, detail="Job not found")
    return status

@app.get("/epitomax_summary/{job_id}")
def get_epitomax_job_summary(job_id: str):
    status = epitomax_job_tracker.get(job_id)
    if not status:
        raise HTTPException(status_code=404, detail="Job not found")
    return {
        ...
        "elapsed_time": (status.end_time - status.start_time).total_seconds() if status.end_time else None,
        "local_path": status.local_path,
        "ftp_host": status.ftp_host,
        "ftp_directory": status.ftp_directory,
